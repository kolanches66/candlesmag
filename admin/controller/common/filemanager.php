...
class ControllerCommonFileManager extends Controller {
	...
	// учитываем ориентацию от камеры
	public function image_orientation($tmp_file, $new_file, $quality = 80) {
		$image_old = imagecreatefromjpeg($tmp_file);
		...
		imagecopyresampled($image_new, $image_old, 0, 0, 0, 0, $width, $height, $width, $height);
		$exif = exif_read_data($tmp_file);
		if(!empty($exif['Orientation'])) {
			switch($exif['Orientation']) {
				case 8:
					$image_new = imagerotate($image_new,90,0);
					break;
				case 3:
					$image_new = imagerotate($image_new,180,0);
					break;
				case 6:
					$image_new = imagerotate($image_new,-90,0);
					break;
		...
		imagejpeg($image_new, $new_file, $quality);
		unlink($tmp_file);
		...
		if (!$json) {
			...
			foreach ($files as $file) {
				if (is_file($file['tmp_name'])) {
					// русские символы переводим в латиницу
					$filename = basename($this->translit(html_entity_decode($file['name'], ENT_QUOTES, 'UTF-8')));

					// фильтруем по длине НАЗВАНИЯ файла
					if ((utf8_strlen($filename) < 3) || (utf8_strlen($filename) > 255)) {
						$json['error'] = $this->language->get('error_filename');
					}
					
					// принимает только эти типы изображений
					$allowed = array(
						'jpg',
						'jpeg',
						'gif',
					...
					// формат не из списка => ошибка
					if (!in_array(utf8_strtolower(utf8_substr(strrchr($filename, '.'), 1)), $allowed)) {
						$json['error'] = $this->language->get('error_filetype');
					}
					
					// принимает только эти mime-типы изображений
					$allowed = array(
						'image/jpeg',
						'image/pjpeg',
						'image/png',
				...
				if (!$json) {
					$tmp_file = $directory . '/tmp_' . $filename;
					$new_file = $directory . '/' . $filename;
					move_uploaded_file($file['tmp_name'], $tmp_file);
					// меняем размер и качество
					$this->image_orientation($tmp_file, $new_file);
				}
			}
		}
